...
class MakeGUI():
    def makegui(self):
        ...
        layout = [
                  ...
                  [sg.InputText('Excel File', font =("Arial", 13), size = (30,1), key='WaterinfoPath'), sg.FileBrowse('SELECT', font =("Arial", 13, 'bold'), size=(10, 1)), sg.Button('Run',font =("Arial", 13, 'bold'), size=(10,1), key='Run')],
...
def preprocess_img(image, savepath, water_info):
    ...
    source = '_'.join(imagenamejpg.split('_')[:3]) + '.mp4'
    video_time = str(datetime.timedelta(seconds= ((int(imagenamejpg.split('_')[3].split('.')[0]))-1)))
    frame_no = (int(imagenamejpg.split('_')[3].split('.')[0])-1)*60

    update_json(savepath + "/" + imagename, imagename, water_info, h, w, source, video_time, frame_no)
